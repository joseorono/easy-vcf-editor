...
export interface VCardRelated {
  type: "spouse" | "child" | "parent" | "sibling" | "friend" | "colleague" | "assistant" | "emergency" | "other"
  value: string
}
...
export const defaultVCardData: VCardData = {
  ...
  impps: [],
  addresses: [
    { type: "home", street: "", city: "", state: "", postalCode: "", country: "", poBox: "", extendedAddress: "" },
  ],
  ...
  for (const line of lines) {
    ...
    switch (key) {
      ...
      case "LOGO":
        ...
        break
      case "EMAIL": {
        const emailType = getTypeFromParams(params, ["work", "home", "other"]) as VCardEmail["type"]
      ...
      case "ADR": {
        const addrType = getTypeFromParams(params, ["work", "home", "other"]) as VCardAddress["type"]
...
export function generateVcf(data: VCardData, version: "3.0" | "4.0" = "4.0"): string {
  ...
  // Structured Name
  lines.push(
    `N:${escapeValue(data.lastName)};${escapeValue(data.firstName)};${escapeValue(data.middleName)};${escapeValue(data.prefix)};${escapeValue(data.suffix)}`,
  )
  ...
  if (data.photo) {
    if (data.photo.startsWith("data:") || data.photo.startsWith("http")) {
      lines.push(version === "4.0" ? `PHOTO:${data.photo}` : `PHOTO;VALUE=URI:${data.photo}`)
    } else {
  ...
  if (data.anniversary) lines.push(`ANNIVERSARY:${formatDateForVcf(data.anniversary)}`)
  ...
  for (const email of data.emails) {
    if (email.value) {
      const typeParam = version === "4.0" ? `TYPE=${email.type}` : `TYPE=INTERNET,${email.type.toUpperCase()}`
  ...
  for (const phone of data.phones) {
    if (phone.value) {
      const typeParam = phone.type === "cell" ? "CELL" : phone.type.toUpperCase()
  ...
  for (const addr of data.addresses) {
    if (addr.street || addr.city || addr.state || addr.postalCode || addr.country) {
      lines.push(
        `ADR;TYPE=${addr.type}:${escapeValue(addr.poBox)};${escapeValue(addr.extendedAddress)};${escapeValue(addr.street)};${escapeValue(addr.city)};${escapeValue(addr.state)};${escapeValue(addr.postalCode)};${escapeValue(addr.country)}`,
      )
    }
  }
  ...
  // Geographic
  if (data.geo) {
    lines.push(version === "4.0" ? `GEO:geo:${data.geo}` : `GEO:${data.geo.replace(",", ";")}`)
  }
  ...
  if (data.calendarAddressUri) lines.push(`CALADRURI:${data.calendarAddressUri}`)
...
export function downloadVcf(data: VCardData, version: "3.0" | "4.0" = "4.0") {
  ...
  const fileName = [data.firstName, data.lastName].filter(Boolean).join("_") || "contact"
  link.href = url
